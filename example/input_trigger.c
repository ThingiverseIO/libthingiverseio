...
int main() {
	...
	trigger(input, "SayHello", sbuf.data, sbuf.size);
	...
	while (available == 0){
		listen_result_available(input, &available);
		printf(".");
		sleep(1);
	}
	...
	char * res_uuid;
	int res_uuid_size;
	retrieve_listen_result_id(input, &res_uuid, &res_uuid_size);

	printf("\nReceived listen result with id %s.\n", res_uuid);

	char * res_fun;
	int res_fun_size;
	retrieve_listen_result_function(input, &res_fun, &res_fun_size);
	...
	retrieve_listen_result_params(input, &rparams, &rparams_size);
